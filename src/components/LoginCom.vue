        ...
        <div class="form-box">
            ...
            <div class="login-box" v-show="loginBox">
                ...
                <form class="personLogin">
                    <input type="email" name="logEmail" placeholder="邮箱" v-model="admin"/>
                    <input type="password" name="logPwd" placeholder="密码" v-model="password"/>
...
import UtilsCom from '@/components/UtilsCom'
...
function register( e ){
	...
	// 发起register 请求
	new request('http://localhost:8082/api/register',{
		email:email.value,
		name:name.value,
...
function sendCode(e) {
    ...
    if( regEmail.test(email.value) ){
		...
		const req = new request('http://localhost:8082/api/sendCode',{email:email.value},false).post()
		req.then( res => {
			console.log(res);
		});
